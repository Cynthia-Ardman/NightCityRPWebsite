...
router.post("/admin/jobs/run", adminOnly, async (req, res): Promise<void> => {
  ...
  if (!["cyberware_humanity", "monthly_rent", "role_sync", "eviction_sweep", "discord_event_sync"].includes(job)) {
    res.status(400).json({ error: "Unknown job" });
    return;
  }
  const result = await runJob(job as "cyberware_humanity" | "monthly_rent" | "role_sync" | "eviction_sweep" | "discord_event_sync");
  res.json(result);
});
