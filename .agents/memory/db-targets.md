...
**Prod hydrated from dev + guards live (2026-06-05):** after the go-live publish (which ran a full drizzle schema push → dev & prod schemas now identical, 72 tables; push also dropped unused `users.active_character_id` from prod), the live prod DB was seeded from dev via `scripts/src/prod-migrate.ts` (phased: content/config/history/sequences/guards). Append-only guards are NOW ACTIVE on prod (6 tables: bot_rent_payment_events, bot_balance_history, bot_actor_attendance, bot_attendance_log, audit_log, activity_events). Rules that mattered: content synced insert-only (ON CONFLICT DO NOTHING) so prod-only rows survive (e.g. inventory_items kept 4 live-only rows); `characters` upserted with `owner_id = COALESCE(prod, dev)` to never un-claim a prod-owned char; `bot_config` insert-missing-keys only (the 6 `*_live_mode` flags landed FALSE — flip them to go live); ALWAYS setval id sequences to MAX(id) after explicit-id copies or next prod insert collides; guards run LAST so a failed insert is still cleanable. `prod-migrate.ts` is executed one-off tooling — do NOT rerun without a fresh `analyze` gate.
