...
reset effect must key on stable character.id, not the character object; amend callers rebuild a new mergedCharacter each render, so object-keying clobbers in-progress image/text amends on any background refetch.
