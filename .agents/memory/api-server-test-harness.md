...
Another observed manifestation of the same aggregate-run flake: `missions.test.ts` fails under `pnpm -r run test` with `Cannot read properties of undefined (reading 'paymentStatus')` + `Failed query: insert into "mission_actor_payments"` — the simulated/live payout rows never insert because the shared test DB lost its schema/seed mid parallel run. The same tests pass in isolation (`vitest run src/routes/missions.test.ts -t "simulated"` → exit 0, zero "Failed query"). Treat as harness, not code.
